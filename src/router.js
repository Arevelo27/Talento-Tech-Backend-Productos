...
router.post("/producto", async (request, response) => {
  ...
  try {
    ...
    const query =
      "INSERT INTO productos (codigo, nombre, descripcion, imagen, edad, activo) VALUES (?, ?, ?, ?, ?, ?)"
...
router.put("/producto/:id", async (request, response) => {
  ...
  try {
    ...
    const query = "UPDATE productos SET codigo = ?, nombre = ?, descripcion = ?, imagen = ?, edad = ?, activo = ? WHERE id = ?"
